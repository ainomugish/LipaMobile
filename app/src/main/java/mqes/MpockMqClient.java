...
//                    }
    ...
    public void connectionLost(Throwable cause) {
        ...
        //SET APP VAR
        MySingleton.getInstance(vars.context).connectionStatus = Globals.DISCONNECTED;
    ...
    public void deliveryComplete(IMqttDeliveryToken token) {
        ...
        //SET SINGLETON VARS
        MySingleton.getInstance(vars.context).connectionStatus = Globals.CONNECTED;

        //CHECK IF USER HAS LOGGED IN
//        if (vars.prefs.getBoolean(Globals.PREFS_LOGGED_IN, false)) {
//            log("Globals.PREFS_LOGGED_IN, IS TRUE, USER IS LOGGED IN");
//
//            SubmitUserOnlineJob suj = new SubmitUserOnlineJob(vars, vars.prefs.getString(Globals.PREFS_USER_NAME, "none_sent"));
//            appVar.jobManager.addJobInBackground(suj);
//        } else if (!vars.prefs.getBoolean(Globals.PREFS_LOGGED_IN, false)) {
//            log("Globals.PREFS_LOGGED_IN, IS FALSE, USER IS NOT LOGGED IN");
//        }
    ...
    public void messageArrived(String topic, MqttMessage message) throws Exception {
        ...
        try {
            //PROCESS MESSAGE...
            if (topic.endsWith("/"+Globals.STUDENT_CHARGE_REPLY)) {
                log("topic.endsWith:"+Globals.STUDENT_CHARGE_REPLY);
                String daMessage = new String(message.getPayload());
                log("daMessage:" + daMessage);

                //GET NEW STUDENT CHARGE OBJECT
//                log("posting NEW_STUDENT_CHARGE");
//                StudentCharge studentCharge = vars.gson.fromJson(daMessage, StudentCharge.class);
//                log("got studentnonorm:"+ studentCharge.lastname);
//                log("got transId:"+ studentCharge.transId);
//                EventBus.getDefault().post(studentCharge);

                log("posting NEW_STUDENT_CHARGE");
                StudentChargeReply scr = vars.gson.fromJson(daMessage, StudentChargeReply.class);
                log("got studentnonorm:"+ scr.studentCharge.lastname);
                log("got transId:"+ scr.studentCharge.transId);
